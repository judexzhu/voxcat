...
- **Persona**: A markdown file (`~/.config/voxcat/personas/*.md`) with YAML frontmatter that shapes the agent's behavior. Includes: system instruction (markdown body), built-in tool set (core + role-specific extras), MCP server set (read-only filtered), voice identity (voice + TTS style), greeting, and output directory. Loaded by `personas.py`, falls back to `config.yaml` profiles if no files exist. Selected via URL parameter.
